...
class Individual():
    def __init__(self, nom, reponses):
#        self.id_number = id_number
    ...
    def copiage2(self,bestF):
        cop1=Individual("",[])
        cop2=Individual("",[])
        cop1.nom = self.nom
        cop1.reponses = self.reponses
        cop1.famille=self.famille
        cop1.preference_list =[]
        cop1.available_proposals =[]
        cop1.partner = None
        cop1.moy_lovesc= -1
        cop1.cop=cop2
        cop2.nom = bestF.nom
        cop2.reponses = bestF.reponses
        cop2.famille=bestF.famille
        cop2.preference_list =[]
        cop2.available_proposals =[]
        cop2.partner = None
        cop2.moy_lovesc= -1
        cop2.cop=cop1
        return(cop1,cop2)
...
ponde = [100,1,1,1,1,1,1,1] #A modifier selon l'ordre des questions
            ...
            for x in r1[k]:
                if x in r2[k]:
                    if x=="Je suis aigri et j'aime pas la musique" :
                        ...
                        s-=p[k]
        elif k ==  2:    #idem pour les centre d'intérêts artistiques
            for x in r1[k]:
                if x in r2[k]:
                    if x=="pas du tout":
                        s-=3*p[k]
                        ...
                        s-=p[k]
        elif k ==  6:    #idem pour le type de sport pratiqué
            for x in r1[k]:
                if x in r2[k]:
                    if x=="Pas de sport":
                        s=s
    ...
    def ecriture(self,membmax):
        ...
        for i in range((nbfam//10)):
            ...
            for j in range(10):
                ...
                s.write(y,x,"Famille : ",style_entete_g)
                s.write(y+1,x,"EI :",style_parrains_ent)
                s.write(y+1,x+1,"Master :",style_fillots_ent)
...
parrains=xlrd.open_workbook("Master Buddy form (EI) (réponses).xlsx")
shnp=parrains.sheet_names()
#print(len(shnp)) #Noms des feuilles
shp=parrains.sheet_by_name(shnp[0]) #Feuille qui m'interresse
...
def ListeParrains():
    ...
    for k in range(1,shp.nrows):
        ...
        EI2+=[Individual(str(l[1])+" "+str(l[2]),l[3:])]
        print(len(l[3:]))
    return(EI2)

fillots=xlrd.open_workbook("Master Buddy form (Master) (réponses).xlsx")
...
def ListeFillots():
    ...
    for k in range(1,shf.nrows):
        l=shf.row_values(k)
        EI1+=[Individual(str(l[1])+" "+str(l[2]),l[3:])]
    return(EI1)
...
def transformationrep(EI):
    for i in EI:
        j=i.reponses
        j[0]=int(j[0].replace("French","0").replace("a foreigner","1"))
        j[1]=j[1].split(", ")
        j[2]=j[2].split(", ")
        j[3]=int(j[3].replace("Once a day","0").replace("many times in a week","1").replace("Once in a while","2").replace("Only at high noon","3"))
        j[4]=int(j[4].replace("Once a day","0").replace("many times in a week","1").replace("Once a week","2").replace("Only on Sundays at 15h23 when it's sunny","3"))
        j[5]=int(j[5].replace("For competition","0").replace("For selfimprouvement","1").replace("For fun","2").replace("For others","3"))
        j[6]=j[6].split(", ")
        j[7]=int(j[7].replace("I always cook good meals","0").replace("I like cooking on week-ends when i have time","1").replace("Oh yeh ! I'm into fat cooking !","2").replace("Does switching between pasta and Macdo count ?","3"))
    return(EI)
...
def algo():
    ...
    for man in P1:
        if man.cop != None :
            cop1,cop2=man.copiage2(man.cop)
            cop3,cop4=man.copiage2(man.cop)
            P.append(cop1)
            P.append(cop2)
            P.append(cop3)
            P.append(cop4)
